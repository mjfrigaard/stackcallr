...
methods reached through
[`UseMethod()`](https://rdrr.io/r/base/UseMethod.html) dispatch.
...
takes the name of a function as a string, bare or namespaced, and
returns the tree of functions it calls. The example below stops two
levels down:
...
For a generic,
[`call_stack()`](https://mjfrigaard.github.io/stackcallr/reference/call_stack.md)
resolves [`UseMethod()`](https://rdrr.io/r/base/UseMethod.html) dispatch
into the actual registered methods, rather than stopping at `UseMethod`
itself. Every `mean.*` node below is a real method, ready to be expanded
further with a larger `max_depth`:
